...
public class JAFController {
	...
	public ResponseEntity<JAF> signJAF(HttpSession hs,@PathVariable(value="jobId") int jobId) {
			//jAF.setjAFId(3);
			jAF.setUserId(hs.getAttribute("loggedInUserId").toString());
			jAF.setStatus('Y');
			jAF.setJobId(jobId);
		if (jAFDAO.saveJAF(jAF)) {	
			...
			jAF.setErrorMsg("Not signed!");
			return new ResponseEntity<JAF>(jAF, HttpStatus.NOT_FOUND);
